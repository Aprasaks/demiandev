...
function useDropdownMenu({
  ...
  const interactions = useInteractions([
    useClick(context, { event: "mousedown", toggle: true, ignoreMouse: false }),
  ...
  const updatePosition = React.useCallback(
    (newSide, newAlign) => {
      const newPlacement = `${newSide}-${newAlign}`;
...
export const DropdownMenuItem = React.forwardRef(({ children, disabled, asChild = false, onSelect, className, ...props }, ref) => {
  ...
  const itemProps = {
    ...
    ...context.getItemProps({ ...props, onClick: handleSelect }),
  };
...
export const DropdownMenuGroup = React.forwardRef(({ children, label, className, ...props }, ref) => (
  <div ref={ref} role="group" aria-label={label} className={`tiptap-button-group ${className || ""}`} {...props}>
    {children}
  </div>
));
